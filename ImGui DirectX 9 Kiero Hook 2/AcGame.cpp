...
void AcGame::Init() {
  this->baseAddress = reinterpret_cast<int>(GetModuleHandle(L"client.dll"));
  Init_base_address();
}

void AcGame::Init_base_address() {
  AC::Base::MatrixAddress = static_cast<Matrixf*>GAME_OFFSET(0x4dc23b4);
}

void AcGame::SetModuleBaseAddress(WCHAR* str) {
  DWORD id = GetCurrentProcessId();
  baseAddress = UTIL::dwGetModuleBaseAddress(id, static_cast<TCHAR*>str);
}
...
void AcGame::DrawBox() {
  for (int i = 1; i < this->numPlayers; i++) {
    ...
    if (!ent) break;
    playerinfo_t playerinfo(ent, static_cast<float>AC::Base::WindowWidth,
                            static_cast<float>AC::Base::WindowHeigth);
...
void AcGame::DrawOnePlayer(playerinfo_t* playerinfo) {
  ...
  ImVec2 TopLeft = {playerinfo->headpos2D.x - BoxWidth / 2,
                    playerinfo->headpos2D.y};
  ImVec2 BottomLeft = {playerinfo->headpos2D.x - BoxWidth / 2,
                       playerinfo->pos2D.y};
  ImVec2 BottomRight = {playerinfo->headpos2D.x + BoxWidth / 2,
                        playerinfo->pos2D.y};
  ImVec2 TopRight = {playerinfo->headpos2D.x + BoxWidth / 2,
                     playerinfo->headpos2D.y};
  ...
  ImColor BoxColor = local_player->team == playerinfo->ent->team
                         ? BoxColor_friend
                         : BoxColor_enemy;
  ...
  TopLeft = ImVec2(BottomLeft.x,
                   BottomLeft.y + (TopLeft.y - BottomLeft.y) * current_health);
  ...
  // TopRight = ImVec2(BottomRight.x, BottomRight.y + (TopRight.y -
  // BottomRight.y) * current_armor);
